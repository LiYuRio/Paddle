                ...
                def _partition_program_to_multiple_blocks(program):
                    ...
                    for op in program.block(0).ops:
                        if op.type == "feed":
                            ...
                            fetch_ops.append(op)
                        elif op.attr("op_role") == 0 or op.attr(
                                "op_role") == 256:
                            forward_ops.append(op)
                        elif op.attr("op_role") == 1 or op.attr(
                                "op_role") == 257:
                            ...
                            print(
                                "Op accidently ignored by event based executor")
                    _add_ops_vars_into_block(
                        new_program.block(0), feed_ops, vars)
                    _add_ops_vars_into_block(new_program._create_block(),
                                             forward_ops, vars)
                    _add_ops_vars_into_block(new_program._create_block(),
                                             backward_ops, vars)
                    _add_ops_vars_into_block(new_program._create_block(),
